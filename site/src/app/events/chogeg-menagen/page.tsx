...
export default function ChogegMenagenPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "Service", name: "חוגג מנגן", provider: { "@type": "Person", name: "Almog Cohen" } }) }} />
      ...
      <div className="border-b border-foreground/10 bg-foreground/5 px-4 py-3">
        <div className="mx-auto max-w-6xl">
          <nav className="flex items-center gap-2 text-sm text-foreground/60">
            ...
            <span>›</span>
            <span className="text-foreground font-medium">חוגג מנגן</span>
          </nav>
        </div>
      </div>
      ...
      <section className="relative overflow-hidden px-4 py-16 md:py-24">
        <div className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute left-0 top-0 h-96 w-96 bg-brand-green/10 blur-[120px]" />
        </div>
          ...
          <div className="grid gap-12 md:grid-cols-2 md:items-center">
            <div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
                <div className="inline-flex items-center gap-2 rounded-full bg-brand-green/20 px-4 py-1">
                  <div className="h-1.5 w-1.5 rounded-full bg-brand-green" />
                  <span className="text-sm font-bold text-brand-green">חוגג מנגן</span>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
                <a href={wa("היי אלמוג, מעוניינים בחוגג מנגן")} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-brand-green to-emerald-500 px-8 py-4 text-lg font-bold text-white transition hover:scale-105 shadow-lg">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 }}
              className="relative aspect-square overflow-hidden rounded-3xl border border-foreground/10 bg-gradient-to-br from-brand-green/20 to-emerald-500/20">
              <div className="absolute inset-0 bg-gradient-to-br from-brand-green/30 to-emerald-500/30" />
              <div className="relative flex h-full items-center justify-center">
                <div className="h-32 w-32 rounded-full bg-gradient-to-br from-brand-green to-emerald-500 blur-3xl opacity-40" />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="px-4 py-16">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {[
              { title: "קבלת פנים", desc: "מוזיקה קלילה ונעימה בכניסה" },
              { title: "ליווי מוזיקלי", desc: "DJ + מוזיקאים לכל שלבי האירוע" },
              { title: "הנחיית אירועים", desc: "מעבר חלק בין השלבים" },
              { title: "רחבת ריקודים", desc: "מוזיקה שמעלה אנרגיה" },
            ].map((item, i) => (
              <div key={i} className="rounded-2xl border border-foreground/10 bg-foreground/5 p-6 transition hover:border-brand-green/30">
      ...
      <section className="px-4 py-24">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <a href={wa("היי אלמוג, רוצים לשמוע על חוגג מנגן")} target="_blank" rel="noopener noreferrer"
