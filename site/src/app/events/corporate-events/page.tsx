...
export default function CorporateEventsPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@type": "Service", name: "DJ לאירועים עסקיים", provider: { "@type": "Person", name: "Almog Cohen" } }) }} />
      ...
      <div className="border-b border-foreground/10 bg-foreground/5 px-4 py-3">
        <div className="mx-auto max-w-6xl">
          <nav className="flex items-center gap-2 text-sm text-foreground/60">
            <Link href="/" className="hover:text-foreground transition">בית</Link>
            <span>›</span>
            <Link href="/events" className="hover:text-foreground transition">אירועים</Link>
            <span>›</span>
            <span className="text-foreground font-medium">אירועים עסקיים</span>
          </nav>
        </div>
      </div>
      ...
      <section className="relative overflow-hidden px-4 py-16 md:py-24">
        <div className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute left-0 top-0 h-96 w-96 bg-indigo-500/10 blur-[120px]" />
        </div>
          ...
          <div className="grid gap-12 md:grid-cols-2 md:items-center">
            <div>
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
                <div className="inline-flex items-center gap-2 rounded-full bg-indigo-500/20 px-4 py-1">
                  <div className="h-1.5 w-1.5 rounded-full bg-indigo-400" />
                  <span className="text-sm font-bold text-indigo-400">אירועים עסקיים</span>
                </div>
              </motion.div>
              ...
              <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-6 text-4xl font-black text-foreground-heading md:text-6xl">
                מוזיקה שמגבשת צוות ומעלה אנרגיות
              </motion.h1>
              ...
              <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-8 text-lg text-foreground-secondary">
                אירוע עסקי הוא <strong className="text-foreground">הזדמנות לחבר</strong> את הצוות מחוץ לשגרה. 
                מסיבות חברה, אירועי השקה, כנסים וימי גיבוש – אני יוצר אווירה שמשחררת ומעלה מורל. 
                <strong className="text-foreground"> מוזיקה שמתאימה לקהל המקצועי ולאופי החברה.</strong>
              </motion.p>

              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mb-8 space-y-3">
                {[
                  "מוזיקה מותאמת לאופי החברה והקהל",
                  "ניסיון באירועי Startup ו-Hitech",
                  "גמישות מלאה - מדוכן שקט ועד רחבת ריקודים",
                  "שירות מקצועי ואמין",
                ].map((item, i) => (
                  <div key={i} className="flex items-center gap-3">
                    <div className="flex h-6 w-6 items-center justify-center rounded-full bg-blue-600/20 text-blue-400">✓</div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 }}
              className="relative aspect-square overflow-hidden rounded-3xl border border-foreground/10 bg-gradient-to-br from-indigo-500/20 to-blue-600/20">
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/30 to-blue-600/30" />
              <div className="relative flex h-full items-center justify-center">
                <div className="h-32 w-32 rounded-full bg-gradient-to-br from-indigo-500 to-blue-600 blur-3xl opacity-40" />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="px-4 py-16">
        <div className="mx-auto max-w-4xl">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {[
              { title: "התאמה אישית", desc: "מוזיקה שמתאימה לאופי החברה" },
              { title: "גמישות", desc: "מרקע מוזיקלי עד רחבת ריקודים" },
              { title: "מקצועיות", desc: "שירות אמין וזמינות גבוהה" },
              { title: "ניסיון", desc: "עבודה עם חברות Startup ו-Hitech" },
            ].map((item, i) => (
              <div key={i} className="rounded-2xl border border-foreground/10 bg-foreground/5 p-6 transition hover:border-indigo-400/30">
